...
const utilsCommandNameSlice = createSlice({
  ...
  reducers: {
      ...
      switch (action.payload.commandName) {
        case 'popUpStatusSuccess':
          ...
          state.actionPopup.textItems = [
            `<strong>Приглашение <br />на рандом-кофе <br /> отправлено <br /></strong>`,`Свяжитесь с коллегой и назначьте место и время встречи`,`<strong>Вкусного кофе и увлекательных бесед.</strong>`
          ];
          break;
    ...
    setIsValentines(state, { payload }) {
      state.isValentines = payload ?? false;
    }
  },
});
